...
class ChatServer:
    def __init__(self, host='0.0.0.0', port=55555):
        ...
        self.user_files = {}  # 用户上传的文件信息 {username: {total_size: size}}
    ...
    def start_server(self):
        if not self.running:
            ...
            try:
                ...
                # 启动文件服务器
                self.start_file_server(port + 1)

                # 获取并显示本机IP地址
                local_ip = get_local_ip()
                self.log_message(f"服务器正在运行 - 本地IP: {local_ip}, 端口: {port}")
                self.log_message(f"文件服务器运行在 - {local_ip}:{port+1}")
        ...
        if self.running:
            ...
            if os.path.exists("./user_files"):
                try:
                    ...
                    self.log_message(f"清理用户文件失败: {str(e)}")
    ...
    def handle(self, client):
        ...
        # 为用户创建文件目录
        user_dir = os.path.join("./user_files", nickname)
        if not os.path.exists(user_dir):
            os.makedirs(user_dir, exist_ok=True)

        # 初始化用户文件大小记录
        if nickname not in self.user_files:
            self.user_files[nickname] = {"total_size": 0}
        ...
        while True and self.running:
            try:
                ...
                if message:
                    ...
                    # 处理文件上传消息
                    if decoded.startswith("/文件上传 "):
                        # 格式: /文件上传 [文件名] [文件大小] [目标用户/公共]
                        parts = decoded[6:].split(" ", 2)
                        if len(parts) == 3:
                            filename, file_size_str, target = parts
                            try:
                                file_size = int(file_size_str)
                                self.handle_file_upload(nickname, filename, file_size, target)
                            except ValueError:
                                client.send("错误: 文件大小必须是一个数字!".encode('utf-8'))
                        else:
                            client.send("格式错误! 使用: /文件上传 文件名 文件大小 目标用户/public".encode('utf-8'))
    ...
    def send_help_message(self, client, nickname):
        ...
        help_msg += "/文件上传 文件名 文件大小 目标用户名/public - 向目标用户或所有人发送文件\n"
        help_msg += "/帮助 - 显示此帮助信息\n"
        ...
        try:
            ...
            class FileServerHandler(http.server.SimpleHTTPRequestHandler):
                ...
                def __init__(self, *args, **kwargs):
                    super().__init__(*args, directory="./user_files", **kwargs)

                def log_message(self, format, *args):
                    # 使用服务器实例的日志方法记录详细信息
                    message = format % args
                    if hasattr(self, 'server_instance'):
                        self.server_instance.log_message(f"文件服务器: {message}")
                    else:
                        print(f"文件服务器: {message}")

                def do_GET(self):
                    """处理GET请求（下载文件）"""
                    try:
                        # 解析请求路径
                        self.log_message("接收GET请求: %s", self.path)

                        # URL解码
                        decoded_path = urllib.parse.unquote(self.path)
                        self.log_message("解码后的路径: %s", decoded_path)

                        # 转换为文件系统路径
                        file_path = self.translate_path(decoded_path)
                        self.log_message("文件系统路径: %s", file_path)

                        # 检查文件是否存在
                        if not os.path.exists(file_path):
                            self.log_message("文件不存在: %s", file_path)
                            self.send_error(404, "文件未找到")
                            return

                        # 处理目录请求
                        if os.path.isdir(file_path):
                            self.log_message("请求的是目录: %s", file_path)
                            # 不允许目录列表
                            self.send_error(403, "禁止访问目录")
                            return

                        # 获取文件大小
                        file_size = os.path.getsize(file_path)
                        self.log_message("文件大小: %d 字节", file_size)
                        ...
                        with open(file_path, 'rb') as f:
                            ...
                            while True:
                                ...
                                try:
                                    ...
                                    self.log_message("发送文件中断")
                                    break

                            self.log_message("文件发送完成: %s", file_path)
                    except Exception as e:
                        self.log_message("处理GET请求错误: %s", str(e))
                ...
                def do_PUT(self):
                    ...
                    try:
                        # 解析请求路径
                        self.log_message("接收PUT请求: %s", self.path)
                        ...
                        # 转换为文件系统路径
                        path = self.translate_path(decoded_path)
                        self.log_message("文件系统路径: %s", path)
                        ...
                        self.log_message("内容长度: %d 字节", content_length)
                        ...
                        with open(path, 'wb') as output_file:
                            ...
                            self.log_message("文件保存完成: %s", path)
                        ...
                        self.log_message("处理PUT请求错误: %s", str(e))
                        self.send_error(500, f"内部服务器错误: {str(e)}")
                ...
                def do_MKCOL(self):
                    ...
                    try:
                        # 解析请求路径
                        self.log_message("接收MKCOL请求: %s", self.path)
                        ...
                        if os.path.exists(path):
                            if os.path.isdir(path):
                                # 目录已存在，返回成功
                                self.log_message("目录已存在: %s", path)
                                self.send_response(200, "Directory already exists")
                                self.send_header('Content-Length', '0')
                                self.send_header('Access-Control-Allow-Origin', '*')  # 允许跨域
                                self.end_headers()
                            else:
                                # 路径存在但不是目录
                                self.log_message("路径存在但不是目录: %s", path)
                                self.send_error(405, "Path exists but is not a directory")
                            return

                        # 创建目录
                        os.makedirs(path, exist_ok=True)
                        self.log_message("目录创建成功: %s", path)

                        # 发送成功响应
                        self.send_response(201, "Directory created")
                        self.send_header('Content-Length', '0')
                        self.send_header('Access-Control-Allow-Origin', '*')  # 允许跨域
                        self.end_headers()

                    except Exception as e:
                        self.log_message("处理MKCOL请求错误: %s", str(e))
                        self.send_error(500, f"内部服务器错误: {str(e)}")
                ...
                def do_OPTIONS(self):
                    ...
                    self.send_header('Access-Control-Allow-Methods', 'GET, PUT, MKCOL, OPTIONS')
                    self.send_header('Access-Control-Allow-Headers', 'Content-Type')
                    self.end_headers()

            # 设置类变量
            FileServerHandler.server_instance = self

            # 配置服务器允许地址重用
            socketserver.TCPServer.allow_reuse_address = True

            # 创建HTTP服务器
            self.file_server = socketserver.ThreadingTCPServer((self.host, port), FileServerHandler)

            # 启动文件服务器线程
            self.file_server_thread = threading.Thread(target=self.file_server.serve_forever)
            self.file_server_thread.daemon = True
            self.file_server_thread.start()

            self.log_message(f"文件服务器已启动在端口 {port}")
            return True
        except Exception as e:
            self.log_message(f"启动文件服务器失败: {str(e)}")
            return False
    ...
    def stop_file_server(self):
        ...
        if self.file_server:
            try:
                ...
                self.log_message(f"停止文件服务器出错: {str(e)}")

    def handle_file_upload(self, sender, filename, file_size, target):
        """处理文件上传请求"""
        try:
            # 检查文件大小限制
            if file_size > 2 * 1024 * 1024 * 1024:  # 2GB
                self.send_message_to_user(sender, f"错误: 文件大小超过2GB限制")
                return False

            # 检查用户总上传限制
            if self.user_files[sender]["total_size"] + file_size > 4 * 1024 * 1024 * 1024:  # 4GB
                self.send_message_to_user(sender, f"错误: 总上传大小超过4GB限制")
                return False

            # 创建文件存储目录
            current_time = time.strftime("%Y%m%d%H%M%S", time.localtime())
            file_dir = os.path.join("./user_files", sender, current_time)
            os.makedirs(file_dir, exist_ok=True)

            # 更新用户文件大小记录
            self.user_files[sender]["total_size"] += file_size

            # 通知对应的用户有新文件上传
            if target.lower() == "public":
                # 公共文件
                self.broadcast(f"[文件] {sender}向所有人发送了一个文件[{filename}]".encode('utf-8'))

                # 为所有用户记录此文件
                for nick in self.nicknames:
                    try:
                        client = self.clients[self.nicknames.index(nick)]
                        file_info = f"/文件信息 {sender} {current_time} {filename} {file_size} public"
                ...
                if target in self.nicknames:
                    target_client = self.clients[self.nicknames.index(target)]

                    # 通知发送者
                    self.send_message_to_user(sender, f"[文件] 您向{target}发送了一个文件[{filename}]")

                    # 通知接收者
                    private_msg = f"[文件] {sender}向您发送了一个文件[{filename}]"
                    target_client.send(private_msg.encode('utf-8'))

                    # 发送文件信息给接收者
                    file_info = f"/文件信息 {sender} {current_time} {filename} {file_size} private"
                    target_client.send(file_info.encode('utf-8'))

                    # 发送文件信息给发送者（自己也能看到自己发的文件）
                    sender_client = self.clients[self.nicknames.index(sender)]
                    sender_client.send(file_info.encode('utf-8'))
                else:
                    self.send_message_to_user(sender, f"错误: 用户 {target} 不存在或未在线")
                    return False

            return True
        except Exception as e:
            self.log_message(f"处理文件上传请求错误: {str(e)}")
            return False
